...
class Network:
    ...
    def _init_graph(self) -> None:
        ...
        with tfutil.absolute_variable_scope(self.scope, reuse=False), tfutil.absolute_name_scope(self.scope):  # ignore surrounding scopes
            ...
            with tf.control_dependencies(None):  # ignore surrounding control dependencies
                if 'multi_images_in' in self.input_names:
                    for name in self.input_names:
                        if name == 'multi_images_in':
                            multi_resolution_num = int(np.log2(self.static_kwargs['resolution'])) -2
                            images_list = [tf.placeholder(tf.float32, name=('%s_res%d' % (name, res))) for res in range(multi_resolution_num)]
                            self.input_templates.append(images_list)
                        else:
                            self.input_templates.append(tf.placeholder(tf.float32, name=name))
                else: self.input_templates = [tf.placeholder(tf.float32, name=name) for name in self.input_names]
        ...
        if 'multi_images_in' not in self.input_names:
        ...
        if 'multi_images_in' in self.input_names:
            self.input_shapes = [t.shape.as_list() for t in self.input_templates[0]]
            self.input_shapes.append(t.shape.as_list() for t in self.input_templates[0:])
        else:
    ...
    def get_output_for(self, *in_expr: TfExpression, return_as_list: bool = False, **dynamic_kwargs) -> Union[TfExpression, List[TfExpression]]:
        ...
        with tfutil.absolute_variable_scope(self.scope, reuse=True), tf.name_scope(self.name):
            ...
            if isinstance(in_expr[0], list):
                names = ['%s_%d' % (self.input_names[0], idx) for idx in range(len(in_expr[0]))]
                names.append(self.input_names[1])
                self.input_names = names
                in_expr_temp = [expr for expr in in_expr[0]]
                in_expr_temp.append(in_expr[1])
                in_expr = in_expr_temp
                multi_res = True
            ...
            if multi_res:
                images = []
                inputs = []
                for idx, res in enumerate(in_expr):
                    if idx < (len(in_expr)-1):
                        images.append(res)
                inputs.append(images)
                inputs.append(final_inputs[-1])
                #final_inputs = inputs
                out_expr = self._build_func(*inputs, **build_kwargs)
            else: out_expr = self._build_func(*final_inputs, **build_kwargs)
