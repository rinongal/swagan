...
def training_loop(
    ...
    with tf.device('/gpu:0'):
        ...
        if resume_pkl is not None:
            ...
            if resume_with_new_nets: G.copy_vars_from(rG); D.copy_vars_from(rD); Gs.copy_vars_from(rGs);del rG; del rGs; del rD;
    ...
    for gpu in range(num_gpus):
        with tf.name_scope('GPU%d' % gpu), tf.device('/gpu:%d' % gpu):
            ...
            with tf.name_scope('DataFetch'):
                ...
                if 'multi_output' in G_args and G_args['multi_output']:
                    reals_read = multi_res_reals(reals_read, training_set.resolution_log2)
    ...
    while cur_nimg < total_kimg * 1000:
        ...
        if cur_tick < 0 or cur_nimg >= tick_start_nimg + sched.tick_kimg * 1000 or done:
            ...
            if image_snapshot_ticks is not None and (cur_tick % image_snapshot_ticks == 0 or done):
                grid_fakes = Gs.run(grid_latents, grid_labels, minibatch_size=sched.minibatch_gpu, truncation_psi=0.5)
