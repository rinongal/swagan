...
from training.additional_losses import sigmoid_focal_crossentropy
...
def D_logistic_r1(G, D, opt, training_set, minibatch_size, reals, labels, gamma=10.0):
    ...
    with tf.name_scope('GradientPenalty'):
        if isinstance(reals, list):
            gradient_penalty = []
            for image in reals:
                real_grads = tf.gradients(tf.reduce_sum(real_scores_out), [image])[0]
                gradient_penalty.append(tf.reduce_sum(tf.square(real_grads), axis=[1, 2, 3]))
...
def G_logistic_ns_pathreg(G, D, opt, training_set, minibatch_size, pl_minibatch_shrink=2, pl_decay=0.01, pl_weight=2.0):
    ...
    with tf.name_scope('PathReg'):
        ...
        if isinstance(fake_images_out, list):
            # Compute |J*y|.
            pl_penalty = []
            for fake_image, shape in zip(fake_images_out,G.output_shapes) :
                pl_noise = tf.random_normal(tf.shape(fake_image)) / np.sqrt(np.prod(shape[2:]))
                pl_grads = tf.gradients(tf.reduce_sum(fake_image * pl_noise), [fake_dlatents_out])[0]
                ...
                with tf.control_dependencies([pl_update]):
                    pl_penalty.append(tf.square(pl_lengths - pl_mean))

            pl_penalty = tf.reduce_mean(pl_penalty)
